...
extern bool set_LR_enable(bool val);
...
void SerialManager::printHelp(void) {  
  ...
  Serial.print(  "   f/F: Enable/Disable LR filter (is_enabled = "); Serial.print(myState.LR_enable); Serial.println(")");
...
bool SerialManager::processCharacter(char c) { //this is called by SerialManagerBase::respondToByte()
  ...
  switch (c) {
    ...
    case 'J':
      ...
      break;      
    case 'f':
      { bool new_val = set_LR_enable(true);
      Serial.print("Recieved: enable LR filter = "); Serial.println(new_val);}
      setNoiseRedButtons();
      break;
    case 'F':
      { bool new_val = set_LR_enable(false);
      Serial.print("Recieved: enable LR filter = "); Serial.println(new_val);}
      setNoiseRedButtons();;
      break;       
    default:
...
void SerialManager::createTympanRemoteLayout(void) {
  ...
  page_h = myGUI.addPage("Noise Learning");
    ...
    card_h = page_h->addCard("LR Filter");  
      card_h->addButton("Enable", "f", "LR_enable",6);  //label, command, id, width
      card_h->addButton("Disable", "F", "",       6);  //label, command, id, width //display the formant shift value
...
void SerialManager::setNoiseRedButtons(bool activeButtonsOnly) {
  ...
  if ((!activeButtonsOnly) || (myState.LR_enable)) setButtonState("LR_enable",myState.LR_enable);
}
